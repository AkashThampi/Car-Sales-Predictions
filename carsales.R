...
Car_sales$Latest.Launch <-
  as.Date(Car_sales$Latest.Launch, "%d-%B-%y")
df <- Car_sales

# Check the NA levels within the different Attributes of the Dataset#
md.pattern(Car_sales)

### eliminating rows having data with missing values except for the 35 obsetvations x4 year.resale value#

na_df <-
  Car_sales[which(is.na(Car_sales$X4.year.resale.value)), ] ## this is valuable information cannot be excluded #

## Eliminating NA's in all the Attributes other than the resale value #

na_col <- c(6:14)# Creating a vector of NA's in all the NA applicable columns except resale value #
...
boruta.train <-
  Boruta(Sales.in.thousands ~ ., data = Car_sales1, doTrace = 20)
print(boruta.train)
...
Car_sales2 <-
  Car_sales1 %>% select(-Vehicle.type,-Latest.Launch,-Fuel.efficiency)

## Dividing data frame into test and train ##

set.seed(123)
inTrain <-
  createDataPartition(Car_sales2$Sales.in.thousands, p = 0.8, list = FALSE)
